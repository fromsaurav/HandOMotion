...
# Camera Settings
wCam, hCam = 640, 480
cap = cv2.VideoCapture(0)  # Change to 1 or higher if you're using an external camera
cap.set(3, wCam)
cap.set(4, hCam)
...
while True:
    ...
    if not success:
        ...
        continue  # Skip this iteration if the frame is not successfully captured
    ...
    if mode == 'Cursor':
        ...
        if fingers[1:] == [0, 0, 0, 0]:  # thumb excluded
            ...
            if len(lmList) != 0:
                ...
                w, h = pyautogui.size()  # Screen size using pyautogui
